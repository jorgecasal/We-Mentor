...
class Checklist {
  ...
  render() {
    ...
    if (this._data.items.length) {
      ...
      const newItem = this.createChecklistItem({ id: id });
  ...
  createChecklistItem(item = {}) {
    ...
    if (item.id) {
      const id = this._make("span", this.CSS.hidden, {
        innerHTML: item.id ? item.id : ""
      });
      checkListItem.appendChild(id);
    }
  ...
  appendNewElement(event) {
     ...
     */
    const newItem = this.createChecklistItem({ id: this.firestoreAutoId() });
  ...
  get CSS() {
    return {
      ...
      hidden: "cdx-checklist__item-hidden"
    };
  }
    ...
    for (let i = 0; i < this._elements.items.length; i++) {
      ...
      if (value) {
        this._data.items.push({
          ...
          id: this._elements.items[i].querySelector(`.${this.CSS.hidden}`)
            .innerHTML
        });
      }
    }
